...
class Welcome extends Front_Controller {
	...
	function index()
	{
		...
		$data['page_title']			= "Pet food online";
		$data['seo_title']			= "Buy online pet food in delhi-ncr";
		$data['homepage']			= true;
		...
		foreach($new_key_list as $key=>$quantity)
		{
			...
			if(!$this->config->item('allow_os_purchase') && (bool)$product['track_stock'])
			{
				...
				if($stock->quantity < $qty_count)
				{
					...
					{
						$response['error'] .= '
<p>'.sprintf(lang('not_enough_stock'), $stock->name, $stock->quantity).'</p>
';
					}
					else
					{
